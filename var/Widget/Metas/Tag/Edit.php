...
class Edit extends Metas implements ActionInterface
{
    ...
    public function insertTag()
    {
        ...
        /** Tin nhắn nhắc nhở */
        Notice::alloc()->set(
            _t('Thẻ/tag <a href="%s">%s</a> đã được tạo.', $this->permalink, $this->name),
            'success'
        );
    ...
    public function form(?string $action = null): Form
    {
        ...
        $name = new Form\Element\Text(
            ...
            null,
            _t('Tên thẻ') . ' *',
            _t('Đây là tên của thẻ/tag, mó sẽ xuất hiện trên blog.')
        );
        $form->addInput($name);
        ...
        $slug = new Form\Element\Text(
            ...
            null,
            _t('URL của thẻ/tag'),
            _t(' URL của thẻ/tag dùng để tạo dạng liên kết thân thiện, nếu để trống tên thẻ sẽ được sử dụng mặc định.')
        );
        $form->addInput($slug);
        ...
        if ('insert' == $action || 'update' == $action) {
            $name->addRule('required', _t('Bạn chưa điền tên thẻ!'));
            $name->addRule([$this, 'nameExists'], _t('Tên thẻ đã tồn tại!'));
            $name->addRule([$this, 'nameToSlug'], _t('Tên thẻ không thể chuyển đổi thành URL của thẻ/tag!'));
            $name->addRule('xssCheck', _t('Vui lòng không sử dụng ký tự đặc biệt trong tên thẻ!'));
            $slug->addRule([$this, 'slugExists'], _t('URL của thẻ/tag đã tồn tại!'));
            $slug->addRule('xssCheck', _t('Vui lòng không sử dụng ký tự đặc biệt trong URL của thẻ/tag!'));
        }
    ...
    public function updateTag()
    {
        ...
        /** Tin nhắn nhắc nhở */
        Notice::alloc()->set(
            _t('Thẻ/tag <a href="%s">%s</a> đã được sửa.', $this->permalink, $this->name),
            'success'
        );
    ...
    public function deleteTag()
    {
        ...
        /** Tin nhắn nhắc nhở */
        Notice::alloc()->set(
            $deleteCount > 0 ? _t('Thẻ/tag đã bị xóa.') : _t('Không có thẻ/tag nào bị xóa.'),
    ...
    public function mergeTag()
    {
        if (empty($this->request->merge)) {
            Notice::alloc()->set(_t('Hãy điền vào các thẻ/tag cần gộp.'), 'notice');
            $this->response->goBack();
        }

        $merge = $this->scanTags($this->request->merge);
        if (empty($merge)) {
            Notice::alloc()->set(_t('Tên thẻ/tag chuẩn bị gộp không hợp lệ.'), 'error');
            $this->response->goBack();
        }
        ...
        if ($tags) {
            ...
            /** Tin nhắn nhắc nhở */
            Notice::alloc()->set(_t('Thẻ/tag đã được gộp.'), 'success');
        } else {
            Notice::alloc()->set(_t('Không có thẻ/tag nào được chọn.'), 'notice');
        }
    ...
    public function refreshTag()
    {
        ...
        if ($tags) {
            ...
            Notice::alloc()->set(_t('Thẻ/tag đã được làm mới.'), 'success');
        } else {
            Notice::alloc()->set(_t('Không có thẻ/tag nào được chọn.'), 'notice');
        }
