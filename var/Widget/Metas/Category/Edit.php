...
class Edit extends Metas implements ActionInterface
{
    ...
    public function form(?string $action = null): Form
    {
        ...
        $slug = new Form\Element\Text(
            ...
            null,
            _t('URL danh mục'),
            _t('URL danh mục được sử dụng để tạo các biểu mẫu liên kết thân thiện. Nên sử dụng các chữ cái, số, dấu gạch dưới và dấu gạch ngang.')
        );
        $form->addInput($slug);
        ...
        $parent = new Form\Element\Select(
            ...
            $this->request->parent,
            _t('Danh mục cha'),
            _t('Danh mục này sẽ được xếp vào danh mục cha mà bạn chọn.')
        );
        ...
        $description = new Form\Element\Textarea(
            ...
            null,
            _t('Mô tả'),
            _t('Sử dụng để thêm mô tả cho danh mục và nó sẽ được hiển thị trong một số chủ đề.')
        );
        ...
        if ('insert' == $action || 'update' == $action) {
            $name->addRule('required', _t('Bạn chưa điền tên danh mục!'));
            $name->addRule([$this, 'nameExists'], _t('Tên danh mục đã tồn tại!'));
            $name->addRule([$this, 'nameToSlug'], _t('Tên danh mục không thể chuyển đổi thành URL danh mục.'));
            $name->addRule('xssCheck', _t('Vui lòng không sử dụng ký tự đặc biệt trong tên danh mục.'));
            $slug->addRule([$this, 'slugExists'], _t('URL danh mục đã tồn tại.'));
            $slug->addRule('xssCheck', _t('Vui lòng không sử dụng ký tự đặc biệt trong URL danh mục.'));
        }
    ...
    public function updateCategory()
    {
        ...
        /** Tin nhắn nhắc nhở */
        Notice::alloc()
            ->set(_t('Sửa danh mục <a href="%s">%s</a> thành công.', $this->permalink, $this->name), 'success');
    ...
    public function mergeCategory()
    {
        ...
        if ($categories) {
            ...
            /** Tin nhắn nhắc nhở */
            Notice::alloc()->set(_t('Các danh mục đã được hợp nhất.'), 'success');
        } else {
    ...
    public function sortCategory()
    {
        ...
        if (!$this->request->isAjax()) {
            ...
            $this->response->throwJson(['success' => 1, 'message' => _t('Phân loại hoàn tất.')]);
        }
    }
    ...
    public function refreshCategory()
    {
        ...
        if ($categories) {
            ...
            Notice::alloc()->set(_t('Đã làm mới danh mục.'), 'success');
        } else {
            Notice::alloc()->set(_t('Không có danh mục nào được chọn.'), 'notice');
        }
    ...
    public function defaultCategory()
    {
        ...
        if ($error = $validator->run($this->request->from('mid'))) {
            ...
            /** Tin nhắn nhắc nhở */
            Notice::alloc()->set(
                _t('Danh mục <a href="%s">%s</a> đã được đặt là mặc định.', $this->permalink, $this->name),
                'success'
            );
        }
    ...
    public function getMenuTitle(): ?string
    {
        ...
        if (isset($this->request->parent)) {
            ...
            if (!empty($category)) {
                return _t('Tạo danh mục con %s', $category['name']);
            }
        ...
        throw new \Typecho\Widget\Exception(_t('Danh mục không tồn tại hoặc đã bị xóa.'), 404);
    }
