...
class Edit extends Users implements ActionInterface
{
    ...
    public function form(?string $action = null): Form
    {
        ...
        /** Tên người dùng */
        $name = new Form\Element\Text('name', null, null, _t('Tài khoản') . ' *', _t('Tài khoản này sẽ là tên để người dùng đăng nhập.')
            . '<br />' . _t('Vui lòng không sao chép các tài khoản hiện có trong hệ thống.'));
        $form->addInput($name);

        /** Địa chỉ email */
        $mail = new Form\Element\Text('mail', null, null, _t('E-mail') . ' *', _t('Địa chỉ email sẽ là thông tin liên hệ chính cho tài khoản này.')
            . '<br />' . _t('Vui lòng không sao chép địa chỉ email đã có trong hệ thống.'));
        $form->addInput($mail);
        ...
        $screenName = new Form\Element\Text('screenName', null, null, _t('Biệt danh'), _t('Biệt danh có thể khác với tên tài khoản và được sử dụng để hiển thị thay cho tên tài khoản.')
            . '<br />' . _t('Nếu bạn để trống phần này, mặc địn tên tài khoản sẽ được sử dụng.'));
        $form->addInput($screenName);

        /** Mật khẩu người dùng */
        $password = new Form\Element\Password('password', null, null, _t('Mật khẩu'), _t('Hãy thiết lập mật khẩu cho tài khoản này.')
            . '<br />' . _t('Nên sử dụng mật khẩu khó đoán gồm các ký tự đặc biệt, chữ cái và số để tăng tính bảo mật cho mật khẩu.'));
        $password->input->setAttribute('class', 'w-60');
        $form->addInput($password);

        /** Xác nhận mật khẩu người dùng */
        $confirm = new Form\Element\Password('confirm', null, null, _t('Xác nhận mật khẩu'), _t('Hãy điền lại mật khẩu một lần nữa.'));
        $confirm->input->setAttribute('class', 'w-60');
        $form->addInput($confirm);

        /** Địa chỉ trang facebook */
        $url = new Form\Element\Text('url', null, null, _t('Địa chỉ Facebook'), _t('Nhập địa chỉ Facebbok của người dùng này, bắt đầu bằng <code>https://facebook.com/*******</code>'));
        $form->addInput($url);
        ...
        $group = new Form\Element\Select(
            ...
            [
                ...
                'contributor' => _t('Mod'), 'editor' => _t('SMOD'), 'administrator' => _t('Admin')
            ],
            null,
            _t('Chức vụ'),
            _t('Các chức vụ khác nhau có quyền khác nhau.') . '<br />' . _t('Hãy đặt là Member.')
        );
        $form->addInput($group);
        ...
        if ('insert' == $action || 'update' == $action) {
            ...
            $url->addRule('url', _t('Địa chỉ Facebook không đúng định dạng!'));
            $mail->addRule('required', _t('Chưa điền địa chỉ Email!'));
        ...
        if ('insert' == $action) {
            $name->addRule('required', _t('Chưa điền tên tài khoản!'));
            ...
            $password->addRule('required', _t('Chưa điền mật khẩu!'));
        }
