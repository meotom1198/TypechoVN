...
class Edit extends PostEdit implements ActionInterface
{
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid)) {
            ...
            if (!$this->have()) {
                throw new Exception(_t('Trang không tồn tại hoặc đã bị xóa.'), 404);
    ...
    public function writePage()
    {
        ...
        if ($this->request->is('do=publish')) {
            ...
            /** 设置提示信息 */
            Notice::alloc()->set(
                _t('Trang "<a href="%s">%s</a>" đã được tạo.', $this->permalink, $this->title),
                'success'
            );
            ...
            if ($this->request->isAjax()) {
                ...
                /** 设置提示信息 */
                Notice::alloc()->set(_t('"%s" đã được lưu dưới dạng bản nháp.', $this->title), 'success');
    ...
    public function markPage()
    {
        ...
        Notice::alloc()
            ->set(
                $markCount > 0 ? _t(' Trang này đã được đánh dấu là <strong>%s</strong>', $statusList[$status]) : _t('Không có trang nào được đánh dấu!'),
    ...
    public function deletePage()
    {
        ...
        Notice::alloc()
            ->set(
                $deleteCount > 0 ? _t('Trang này đã bị xóa!') : _t('Không có trang nào bị xóa!'),
    ...
    public function sortPage()
    {
        ...
        if (!$this->request->isAjax()) {
            ...
            $this->response->throwJson(['success' => 1, 'message' => _t('Đã sắp xếp trang.')]);
        }
    }
