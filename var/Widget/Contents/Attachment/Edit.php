...
class Edit extends PostEdit implements ActionInterface
{
    ...
    public function form(): Form
    {
        ...
        /** Tên tập tin */
        $name = new Form\Element\Text('name', null, $this->title, _t('Tên tập tin') . ' *');
        $form->addInput($name);
        ...
        $slug = new Form\Element\Text(
            ...
            $this->slug,
            _t('URL của tập tin'),
            _t('URLChữ viết tắt của tập tin được sử dụng để tạo các biểu mẫu liên kết thân thiện. Nên sử dụng các chữ cái, số, dấu gạch dưới và dấu gạch ngang.')
        );
        $form->addInput($slug);
        ...
        /** nút gửi */
        $submit = new Form\Element\Submit(null, null, _t('Lưu lại'));
        ...
        $slug->addRule([$this, 'slugExists'], _t('URL của tập tin đã tồn tại!'));
