...
class Edit extends Contents implements ActionInterface
{
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid)) {
            ...
            if (!$this->have()) {
                throw new Exception(_t('Bài viết không tồn tại hoặc đã bị xóa.'), 404);
    ...
    public function writePost()
    {
        ...
        if ($this->request->is('do=publish')) {
            ...
            Notice::alloc()->set('post' == $this->type ?
                _t('Bài viết "<a href="%s">%s</a>" đã được đăng.', $this->permalink, $this->title) :
            ...
            if ($this->request->isAjax()) {
                ...
                /** 设置提示信息 */
                Notice::alloc()->set(_t('Bài viết "%s" đã được lưu thành bản nháp.', $this->title), 'success');
    ...
    public function markPost()
    {
        ...
        $statusList = [
            'publish' => _t('Đăng/Công Khai'),
            'private' => _t('Bài viết riêng tư'),
            'hidden'  => _t('Ẩn bài viết'),
            'waiting' => _t('Chờ kiểm duyệt')
        ];
        ...
        Notice::alloc()
            ->set(
                $markCount > 0 ? _t('Bài viết đã được gắn thẻ là <strong>%s</strong>', $statusList[$status]) : _t('Không có bài viết nào được gắn thẻ.'),
