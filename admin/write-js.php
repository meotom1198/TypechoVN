...
$(document).ready(function() {
    ...
    if (tags.length > 0) {
        ...
        ?>, {
            ...
            hintText        :   '<?php _e('Vui lòng nhập tên thẻ/tags.'); ?>',
            noResultsText   :   '<?php _e('Thẻ/tags này không tồn tại, nhấn Enter để tạo.'); ?>',
            prePopulate     :   tagsPre,
    ...
    function saveData(cb) {
        ...
        autoSave.text('<?php _e('Đang tự động lưu bài viết...'); ?>');
...
<?php if ($options->autoSave): ?>
    ...
    $('#text').bind('input propertychange', function () {
        if (!locked) {
            autoSave.text('<?php _e('Chưa lưu'); ?>' + (lastSaveTime ? ' (<?php _e('Lưu lần cuối'); ?>: ' + lastSaveTime + ')' : ''));
        }
...
<?php endif; ?>
    ...
    $(window).bind('beforeunload', function () {
        if (changed && !submitted) {
            return '<?php _e('Nội dung bài viết đã được thay đổi và chưa được lưu. Bạn có chắc chắn muốn rời khỏi trang này không?'); ?>';
        }
    });
    ...
    btnPreview.click(function () {
        if (changed) {
            ...
            if (confirm('<?php _e('Nội dung bài viết đã thay đổi cần được lưu lại làm bán nháp trước, sau đó bạn có thể xem trước. Bạn có muốn lưu lại ngay không?'); ?>')) {
                saveData(function (o) {
